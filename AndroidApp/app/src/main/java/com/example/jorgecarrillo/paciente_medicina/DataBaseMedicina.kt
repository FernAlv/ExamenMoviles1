...
class DataBaseMedicina {
    ...
    companion object {
        ...
        fun updateMedicina(medicina: Medicina) {
            "http://192.168.100.62:1337/Medicina/${medicina.id}".httpPut(listOf("gramosAConsumir" to medicina.gramosAConsumir, "nombre" to medicina.nombre, "numeroPastillas" to medicina.numeroPastillas, "composicon" to medicina.composicion, "fechaCaducidad" to medicina.fechaCaducidad, "usadaPara" to medicina.usadaPara))
                    .responseString { request, _, result ->
                        Log.d("http-ejemplo", request.toString())
                    }
        }
        ...
        fun deleteMedicina(id: Int) {
            "http://192.168.100.62:1337/Medicina/$id".httpDelete()
        ...
        fun getMedicinaList(idPaciente: Int): ArrayList<Medicina> {
            ...
            val (request, response, result) = "http://192.168.100.62:1337/Medicina?pacienteId=$idPaciente".httpGet().responseString()
        ...
        fun getList(): ArrayList<Medicina> {
            ...
            val policy = StrictMode.ThreadPolicy.Builder().permitAll().build()
            StrictMode.setThreadPolicy(policy)
            val (request, response, result) = "http://192.168.100.62:1337/Medicina".httpGet().responseString()
